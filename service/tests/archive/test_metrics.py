...
def test_clean_run_reports_zero_pending():
    ...
    assert 'archive_days_pending{mode="metro"} 0' in text
    assert 'archive_days_pending_oldest_age_days{mode="metro"} 0' in text
    assert f'archive_last_run_timestamp_seconds{{mode="metro"}} {NOW.timestamp()}' in text
    assert 'archive_upload_retry_failures_total{mode="metro"} 0' in text
...
def test_pending_days_report_oldest_age():
    ...
    assert 'archive_days_pending{mode="metro"} 2' in text
    assert 'archive_days_pending_oldest_age_days{mode="metro"} 28' in text
    assert 'archive_upload_retry_failures_total{mode="metro"} 3' in text


def test_mode_label_distinguishes_instances():
    text = render_textfile(_result(failed=(date(2026, 7, 1),)), NOW, mode="vline")
    assert 'archive_days_pending{mode="vline"} 1' in text
    assert 'mode="metro"' not in text
...
def test_write_textfile_metrics_creates_parent_and_is_readable(tmp_path):
    ...
    assert path.exists()
    assert 'archive_days_pending{mode="metro"} 0' in path.read_text()
